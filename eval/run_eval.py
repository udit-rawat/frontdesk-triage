...
def main() -> int:
    ...
    for case in cases:
        ...
        ok = cat_ok and pri_ok and own_ok and conf_ok
        passed += ok
        rows.append(
            (case["id"], "PASS" if ok else "FAIL",
             ...
             f"{t.confidence:.2f}{'' if conf_ok else ' !'}", out["source"], f"{out['latency_ms']}ms", case["label"])
        )

    header = ("ID", "", "CATEGORY", "PRIORITY", "OWNER", "CONF", "SOURCE", "TIME", "CASE")
